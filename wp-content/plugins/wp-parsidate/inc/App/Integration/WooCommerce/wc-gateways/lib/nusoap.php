...
class nusoap_base {
      ...
      case is_object( $val ):
        ...
        if ( get_class( $val ) == 'soapval' ) {
          ...
          foreach ( get_object_vars( $val ) as $k => $v ) {
            $pXml = isset( $pXml ) ? $pXml . $this->serialize_val( $v, $k, false, false, false, false,
                $use ) : $this->serialize_val( $v, $k, false, false, false, false, $use );
          }
        }
        if ( isset( $type ) && isset( $type_prefix ) ) {
          $type_str = " xsi:type=\"$type_prefix:$type\"";
  ...
  function serializeEnvelope(
    ...
    $this->debug( "In serializeEnvelope length=" . strlen( $body ) . " body (max 1000 characters)=" . substr( $body, 0,
        1000 ) . " style=$style use=$use encodingStyle=$encodingStyle" );
    $this->debug( "headers:" );
  ...
  function setCredentials(
    ...
    if ( $authtype == 'basic' ) {
      $this->setHeader( 'Authorization',
        'Basic ' . base64_encode( str_replace( ':', '', $username ) . ':' . $password ) );
      ...
      if ( isset( $digestRequest['nonce'] ) ) {
        ...
        if ( $digestRequest['qop'] != '' ) {
          $unhashedDigest = $HA1 . ':' . $nonce . ':' . sprintf( "%08d",
              $digestRequest['nc'] ) . ':' . $cnonce . ':' . $digestRequest['qop'] . ':' . $HA2;
        } else {
        ...
        $this->setHeader( 'Authorization',
          'Digest username="' . $username . '", realm="' . $digestRequest['realm'] . '", nonce="' . $nonce . '", uri="' . $this->digest_uri . $opaque . '", cnonce="' . $cnonce . '", nc=' . sprintf( "%08x",
...
class nusoap_server extends nusoap_base {
  ...
  function serialize_return() {
    ...
    if ( $this->wsdl ) {
      ...
      $this->responseSOAP = $this->serializeEnvelope( $payload, $this->responseHeaders, $this->wsdl->usedNamespaces,
        $this->opData['style'], $this->opData['output']['use'], $encodingStyle );
    } else {
  ...
  function register(
    ...
    if ( $this->wsdl ) {
      $this->wsdl->addOperation( $name, $in, $out, $namespace, $soapaction, $style, $use, $documentation,
        $encodingStyle, $customResponseTagName );
    }
...
class wsdl extends nusoap_base {
      ...
      if ( is_array( $value ) && sizeof( $value ) >= 1 ) {
        ...
        foreach ( $value as $v ) {
          ...
          if ( ! in_array( $typeDef['arrayType'], $this->typemap['http://www.w3.org/2001/XMLSchema'] ) ) {
            ...
            $contents .= $this->serialize_val( $v, 'item', $typeDef['arrayType'], null, $this->XMLSchemaVersion, false,
              $use );
          }
        }
      } else {
          ...
          if ( isset( $attrs['maxOccurs'] ) && ( $attrs['maxOccurs'] == 'unbounded' || $attrs['maxOccurs'] > 1 ) && isset( $v ) && is_array( $v ) && $this->isArraySimpleOrStruct( $v ) == 'arraySimple' ) {
            ...
            foreach ( $vv as $v ) {
              if ( isset( $attrs['type'] ) || isset( $attrs['ref'] ) ) {
                ...
                $xml .= $this->serializeType( $eName, isset( $attrs['type'] ) ? $attrs['type'] : $attrs['ref'], $v,
                  $use, $encodingStyle, $unqualified );
              } else {
  ...
  function addComplexType(
    ...
    $this->schemas[ $typens ][0]->addComplexType( $name, $typeClass, $phpType, $compositor, $restrictionBase, $elements,
      $attrs, $arrayType );
  }
...
class nusoap_parser extends nusoap_base {
      ...
      if ( ! empty( $this->attachments ) ) {
        ...
        foreach ( $this->attachments as $key => $attachment ) {
          $startPos  = max(
            ...
            ( array_key_exists( 'Content-Type', $attachment ) ? stripos( $substrXml,
              $attachment['Content-Type'] ) : 0 ),
            ( array_key_exists( 'Content-Id', $attachment ) ? stripos( $substrXml, $attachment['Content-Id'] ) : 0 ),
            ( array_key_exists( 'Content-Transfer-Encoding', $attachment ) ? stripos( $substrXml,
              $attachment['Content-Transfer-Encoding'] ) : 0 )
          );
          ...
          $length    = null;
          if ( array_key_exists( $key + 1,
              $this->attachments ) && $this->attachments[ $key + 1 ] && ! empty( $this->attachments[ $key + 1 ]['boundaryStr'] ) ) {
  ...
  function character_data( $parser, $data ) {
    ...
    if ( $this->xml_encoding == 'UTF-8' ) {
      ...
      if ( $this->decode_utf8 ) {
        $data = function_exists( 'mb_convert_encoding' ) ? mb_convert_encoding( $data, 'ISO-8859-1',
          'UTF-8' ) : utf8_decode( $data );
      }
    }
    $this->message[ $pos ]['cdata'] .= $data;
    // for doclit
...
class nusoap_client extends nusoap_base {
  ...
  function call(
    ...
    $this->debug( 'SOAP message length=' . strlen( $soapmsg ) . ' contents (max 1000 bytes)=' . substr( $soapmsg, 0,
        1000 ) );
    // send
